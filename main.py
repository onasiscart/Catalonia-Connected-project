...
def read_quality() -> tuple[int, float]:
    ...
    if quality == "1":
        print(f"Select the number of clusters:", end="")
        n_clusters = read(int)
        print(f"Select the minimum angle between segments in the graph:", end="")
        epsilon = read(float)
...
def read_start_point() -> Point:
    print("\nIntroduce the coordinates where you request to start the route:")
...
def main() -> None:
    ...
    elif settings.requested_maps == "all" or settings.requested_maps == "graph":
        graph = get_graph(get_segments(settings.map_zone, settings.files['data']), settings.clusters, settings.epsilon)
        # routes = routes.find_routes(graph, start, get_monuments())
